...
- 文档版本正文读取统一通过 `GET /api/v1/documents/{documentId}/content` 完成，并通过必填查询参数 `source` 表达读取意图
- `source=LATEST` 用于文档详情 latest 正文读取；`source=ASKABLE_BASELINE` 用于问答引用侧栏正文核对；`source=EXPLICIT_VERSION&versionNumber={versionNumber}` 用于管理人员读取指定历史版本正文
...
- 当当前最新版本处于 `UPLOADED`、`INGESTING` 时，应禁止再次发起“上传新版本”
- 当当前最新版本处于 `UPLOADED`、`INGESTING` 时，应禁止发起“版本回退”
- 当当前最新版本处于 `UPLOADED`、`INGESTING` 时，应继续禁止发起“删除 document”
- 当当前最新版本处于 `FAILED` 时，“上传新版本”与“重处理”可同时作为入口开放，但任一动作一旦启动，另一个应因状态变化立即关闭
- 当当前最新版本处于 `FAILED` 时，“版本回退”也可开放，但应与“上传新版本”“重处理”保持互斥，任一动作启动后其它入口立即失效
- `FAILED` 状态下的治理动作并发请求应统一拒绝，不做后台排队；用户需刷新详情后重新决策
- 当 `document` 处于 `DELETING` 时，应禁止上传新版本、版本回退、重处理与问答入口
- 当 `document` 已 `DELETED` 时，应禁止所有版本治理动作与问答入口，但仍允许通过状态查询看到 `DELETED` 终态信息
- 当 `document` 已 `DELETED` 时，具备目标 `document` 管理权限的用户仍可查看其历史版本，仅用于审计/治理查看，不恢复回退、问答或上传新版本能力
...
- 版本历史列表默认折叠
- 版本历史列表默认按最新版本在前排序
- 版本历史列表每项默认展示 `versionNumber`、来源文件名、上传时间、上传人、中文状态与当前是否可用于问答
- 版本历史列表默认展示每个版本的来源文件名
- 版本历史列表默认展示上传时间和上传人
- 版本历史列表默认展示每个版本的中文状态文案
- 版本历史列表默认展示每个版本当前是否可用于问答
- 版本历史列表中的标记体系固定为：`最新`、`可问答`、`回退产生`、`曾被回退为最新版本`
- 同一版本可同时出现多个标记，标记展示顺序固定为：`最新`、`可问答`、`回退产生`、`曾被回退为最新版本`
- `可问答` 标记由当前问答基线规则推导，不作为独立持久化事实
- 版本历史列表对 `FAILED` 版本默认展示简短失败原因摘要，其它状态不展示额外失败摘要
- 上传新版本成功后，版本历史列表应自动展开一次，便于用户立即核对新旧版本关系
- 版本回退成功后，版本历史列表也应自动展开一次，便于用户立即核对回退产生的新旧版本关系
...
- 差异摘要默认比较“当前正在查看的历史版本”与“当前最新版本”，而不是与相邻上一个版本比较
- 历史版本与最新版本的差异提示默认显示在详情页主视图顶部，而不是仅嵌在版本历史列表项内
- 当当前查看的就是最新版本时，不展示差异摘要区
...
当前前端控制台应按“**外层模块型导航 + 页面内任务型编排**”理解，而不是纯流程型站点，也不是纯资源 CRUD 台账。

- 当前已落地的稳定事实：
    - 一级导航已按 `文档 / 知识库 / 问答 / 系统管理` 四类模块收口
    - 共享页面骨架固定为页头、摘要区、主工作区和状态区
    - `knowledge` 页已作为真实接入路径验证共享骨架，不是停留在抽象组件
    - `loading / empty / error / ready` 已统一为共享状态卡表达
    - 控制台外层已补齐 `skip link / nav / main` 可访问性骨架

- 一级导航保持模块型组织，当前主入口应围绕 `文档`、`知识库`、`问答`、`系统管理` 这类稳定模块展开
- 页面内信息编排按任务型组织，优先让用户在当前页面内完成“进入、核对、继续下一步”的闭环
- 共享页面优先服务普通成员的主任务，再对更高权限角色自然展开更深的信息与治理能力
- 纯治理页面不伪装成普通页；成员、账号、授权、审计等页面直接按治理工作台设计

当前前端角色体验不应被简化成“普通用户 / 管理员”二分，而应至少区分三层：

- `KB_READER`：核对层，关注问答、引用、问答基线正文与只读文档查看
- `KB_CONTRIBUTOR` / `KB_MANAGER`：内容治理层，关注上传、版本、重处理、回退、删除与文档治理
- `WORKSPACE_ADMIN` / `WORKSPACE_OWNER`：系统治理层，关注成员、账号、授权、审计与工作区级治理
...
文档、知识库、问答三类共享页面应遵守以下主叙事：

- 问答页的主叙事是“我要问什么，系统基于什么回答，我如何核对依据”
- 知识库页的主叙事是“我该进入哪个 `knowledge base` 开始问答或核对”
- 文档详情页对普通成员的主叙事是“这份文档当前用于问答的依据是什么”，对治理角色的扩展叙事才是版本与治理动作
...
**视觉与组件实现约束：**
当前项目的所有前端界面设计、组件实现与视觉交互，必须严格遵循根目录下的 `DESIGN.md` (Unified RAG Design System)。无论是针对管理者的 Supabase 风格高密度面板，还是针对普通用户的 Cursor 风格阅读/问答流，均禁止随意引入阴影、禁止使用非标准的配色方案，并须严格遵守 `JetBrains Mono` 作为代码和数据渲染字体的约定。
