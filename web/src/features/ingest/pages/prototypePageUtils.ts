// Prototype page utility functions — extracted for Fast Refresh compatibility
import type { PrototypeDocument, PrototypeVersion, ResultState, PrototypeViewModel } from "./IngestDocumentDetailPrototypePage";
...
export function buildPrototypeDocument(documentId: string): PrototypeDocument {
	return {
		documentId,
		kbId: "kb-policy-lab",
		title: "采购制度汇编（2026 版）",
...
export function resultMeta(resultState: ResultState, model: PrototypeViewModel) {
	switch (resultState) {
		case "upload":
			return {
				type: "success" as const,
				title: `已创建新版本 v${model.document.latestVersionNumber}`,
				description: `上一版本为 v6，当前详情页已切换到最新版本；问答基线暂时仍落在 v${model.document.askableVersionNumber}。`,
			};
		case "reuse":
			return {
				type: "info" as const,
				title: "未创建新版本",
				description: `上传文件与当前最新版本内容一致，系统仍停留在 v${model.document.latestVersionNumber}。`,
			};
		case "rollback":
			return {
				type: "warning" as const,
				title: `已创建新的最新版本 v${model.document.latestVersionNumber}`,
				description: `该提示用于预览"回退成功后的稳定结果区"，强调它回退自历史版本 v4，并可能改变问答基线。`,
			};
		default:
			return null;
	}
