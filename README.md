...
- 前端控制台（`web/`，截至 2026-05-16）：
    - React 19 + TypeScript 6 + Vite 8 + Ant Design 6
    - 登录页 + 控制台页，按路由懒加载代码拆分
    - 登录后默认落点按用户能力位自动选择
    - 左侧边栏按能力位渲染，并已收口为 `文档 / 知识库 / 问答 / 系统管理` 四类稳定模块入口
    ...
    - 共享页面骨架已形成：页头 / 摘要区 / 主工作区 / 状态区；`knowledge` 页已作为真实接入路径验证
    - 共享状态表达已统一到 `loading / empty / error / ready` 状态卡
...
- 当前页面范围：
    ...
    - `qa`
- 当前控制台能力：
    - 外层模块型导航 + 页面内任务型编排的共享壳层
    - 文档列表、文档上传、状态查询、分块预览、重处理、删除
    - 知识库主数据管理与跳转问答
    - 单轮问答结果与引用来源展示
    - 成员、账号、知识库授权、文档授权、审计日志治理工作台
    - 统一状态卡（`loading / empty / error / ready`）与可访问性基线（`skip link / nav / main`）
