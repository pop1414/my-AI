...
2. Controller 校验请求并组装 `AcceptUploadCommand`
3. Application 生成 `DocumentId`，创建 `Document(UPLOADED)` 并通过 `DocumentRepository` 落库
4. Controller 返回 `documentId + ACCEPTED`
5. 客户端调用 `GET /api/v1/documents/{id}/status`
6. Application 通过 Repository 查询并返回当前状态；未命中返回 404
