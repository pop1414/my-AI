...
- 新增控制台共享页面骨架基线：固定页头、摘要区、主工作区、状态区四段结构，并在 `knowledge` 页完成真实接入验证
- 新增共享状态卡：统一承接 `loading / empty / error / ready` 页面状态
- 新增控制台可访问性骨架：`skip link`、语义化 `nav` 与 `main`、链接型导航按钮
...
- 控制台导航从页面平铺入口重构为模块型导航：一级稳定收口为 `文档 / 知识库 / 问答 / 系统管理`
...
- 文档列表操作区从纯图标按钮调整为语义化字段按钮，统一承接查看详情、分块预览、重处理、删除和授权管理动作
- 文档、治理与状态相关页面统一增强“返回 xxx”按钮样式，减少回退动作被弱化的问题
...
- QA 版本链收口（2026-05-15）：#10 已关闭，后端问答已接入按文档独立选择可问答版本、版本化引用字段与 stale 汇总；相关公开文档和 API 契约已同步。
- QA 版本链收口（2026-05-15）：#11 已关闭，问答页已接入引用版本卡片、stale reference 顶部提示、无引用兜底提示规则和顶栏问答入口；PGVector `ISNULL` 兼容问题已修复并补齐回归测试。
...
- 控制台壳层收口（2026-05-16）：#33 已完成，前端控制台已形成模块型外层导航、共享页面骨架、统一状态表达与基础可访问性结构；相关前端测试与公开文档已同步。
