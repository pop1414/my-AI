...
1. `docs: 明确 S3 兼容文档资产存储方案`
2. `feat(ingest): 增加 S3 存储配置与客户端装配`
3. `feat(ingest): 实现 S3 source 存储适配器`
4. `feat(ingest): 实现 S3 artifact 存储适配器`
5. `test(ingest): 补齐 S3 存储适配器测试`
6. `chore(infra): 增加 RustFS 本地开发环境`
7. `docs(runbook): 增加 RustFS 运维与排障说明`
8. `docs(plan): 编写历史本地文件迁移计划`
