...
后续如果优化图片、表格、OCR 或复杂版式，也应先把样本纳入本闭环，再讨论是否需要升级节点契约。

推荐做法：

- 仍然先记录 `cleaned.md` 是否保留了可读文字锚点。
- 再记录 chunk preview 的边界和 `sourceHint` 是否更可解释。
- 最后记录固定 `qa.ask` 是否更容易命中预期位置。

如果某个优化必须依赖图片 OCR 结果、表格单元格结构、父子节点或新的引用字段才能证明价值，就说明它已经超出当前 `cleaned.md` 纯文字闭环，应先补新的方案文档或 ADR。
