...
- 拉取方式：基于 GitHub Issues 当前状态更新；#1、#2、#3、#4、#5、#19、#20 已关闭，版本历史只读前后端基线与上传新版本前后端闭环已完成
- 仓库：`pop1414/my-AI`
- 范围：文档版本链相关 issues；#1、#2、#3、#4、#5、#19、#20 已完成关闭，当前仍需推进 #6-#12，共 7 个 open follow-up
...
7. 前端在后端契约稳定后推进 #7、#9、#11；#7 可基于 #3 已落地的版本历史只读视图接入回退动作，并参考 #5 的稳定结果提示模式。
...
- 特别检查 #19 是否足以阻止后续代码继续依赖主表旧版本事实字段，#20 是否足以支撑 #3 不靠 mock 或临时接口推进，#3 和 #5 是否足以作为 #7 回退前端交互的只读页面与稳定结果提示基线。
