 ...
 * @since 1.0.0
 */
public interface DocumentGrantManagementRepository {
     ...
     */
    Optional<DocumentGrant> findActiveGrant(String workspaceId, String documentId, String userId);

    /**
     * 授予或更新文档级授权（Upsert 语义）。
     * <p>
     * 若指定文档与用户的授权记录已存在，则更新权限和状态为 {@code ACTIVE}；
     * 否则插入新记录。此方法不返回布尔值，调用方通过后续查询确认结果。
     *
     * @param workspaceId 工作区 ID，不能为空
     * @param documentId  文档 ID，不能为空
     * @param userId      用户 ID，不能为空
     * @param permission  文档权限枚举值，不能为空
     * @param updatedAt   操作时间戳，用于审计追踪
     */
    void saveGrant(String workspaceId, String documentId, String userId, DocumentPermission permission, Instant updatedAt);
     ...
     */
    boolean disableGrant(String workspaceId, String documentId, String userId, Instant updatedAt);
}
