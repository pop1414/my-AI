 ...
 * 回收文档授权命令。
 * <p>
 * 封装回收文档级授权操作所需的全部入参（文档 ID + 用户 ID），
 ...
 * @since 1.0.0
 */
public record RevokeDocumentGrantCommand(
        String documentId,
        String userId) {
     ...
     */
    public RevokeDocumentGrantCommand {
        // 校验文档 ID 非空且非纯空白
    ...
    /**
     * 获取去除首尾空白后的文档 ID，用于仓储层精确匹配。
     *
     * @return 规范化（trim）后的文档 ID
     */
