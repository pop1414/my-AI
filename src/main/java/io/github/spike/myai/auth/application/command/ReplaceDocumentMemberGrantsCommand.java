 ...
 * 替换文档成员授权命令。
 * <p>
 * 用于一次性全量替换指定文档的所有成员授权关系。
 * 该命令接收一个文档ID和一组用户-权限分配列表，
 * 执行后将覆盖该文档下已有的所有成员权限。
 * </p>
 *
 * @param documentId  目标文档的唯一标识，不可为空
 * @param assignments 授权分配列表，包含用户ID与对应权限；为 null 时视为空列表
 * @author spike
 */
public record ReplaceDocumentMemberGrantsCommand(
        String documentId,
     ...
     * 紧凑构造器，用于参数校验与防御性拷贝。
     * <ul>
     *   <li>校验 {@code documentId} 不为空；</li>
     *   <li>若 {@code assignments} 为 {@code null}，则初始化为不可变空列表；
     *   <li>否则进行不可变拷贝，防止外部修改。</li>
     * </ul>
     */
    public ReplaceDocumentMemberGrantsCommand {
    ...
    /**
     * 授权分配记录，表示一名用户在一个文档上的权限。
     *
     * @param userId     用户唯一标识
     * @param permission 文档权限字符串，需可解析为 {@link DocumentPermission}
     */
    public record Assignment(String userId, String permission) {
        /**
         * 紧凑构造器，校验用户ID和权限字符串均不为空。
         */
        public Assignment {
