 ...
 * @since 1.0.0
 */
public interface RevokeDocumentGrantUseCase {

    /**
     * 执行回收文档授权用例。
     *
     * @param command 回收命令，包含文档 ID 和目标用户 ID，不能为 {@code null}
     * @throws io.github.spike.myai.auth.application.exception.DocumentGrantNotFoundException 当目标授权记录不存在时抛出
     * @throws IllegalArgumentException                                               当命令中任一必填字段为空时抛出
     */
    void handle(RevokeDocumentGrantCommand command);
}
