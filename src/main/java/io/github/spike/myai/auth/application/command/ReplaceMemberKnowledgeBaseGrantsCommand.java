 ...
 * 替换成员知识库授权命令。
 * <p>
 * 用于一次性全量替换指定用户在所有知识库中的角色分配。
 * 该命令以用户ID为主视角，接收一组知识库-角色分配列表，
 * 执行后将覆盖该用户已有的所有知识库角色。
 * </p>
 *
 * @param userId      目标用户的唯一标识，不可为空
 * @param assignments 授权分配列表，包含知识库ID与对应角色；为 null 时视为空列表
 * @author spike
 */
public record ReplaceMemberKnowledgeBaseGrantsCommand(
        String userId,
     ...
     * </ul>
     */
    public ReplaceMemberKnowledgeBaseGrantsCommand {
